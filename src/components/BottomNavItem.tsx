...
export function BottomNavItem({icon:Icon, label, active, onClick, badge}:Props) {
  return (
    <m.button onClick={onClick} whileTap={{scale:0.92}} style={{
      ...
      padding:"7px 2px 8px", display:"flex", flexDirection:"column", alignItems:"center", gap:3,
      WebkitTapHighlightColor:"transparent",
    }}>
      <div style={{position:"relative", width:40, height:26, borderRadius:radii.badge,
        display:"flex", alignItems:"center", justifyContent:"center",
        background: active ? withAlpha(colors.primary,"soft") : "transparent",
        boxShadow: active ? `0 0 10px ${withAlpha(colors.primary,"medium")}` : "none",
        transition:"background .15s, box-shadow .15s"}}>
        <Icon size={18} color={active?colors.primaryLight:colors.textMuted} strokeWidth={active?2.3:1.75}/>
      ...
      <span style={{fontSize:9, fontWeight:active?800:700, letterSpacing:0.6,
        color:active?colors.primaryLight:colors.textMuted, textTransform:"uppercase"}}>{label}</span>
    </m.button>
  );
}
