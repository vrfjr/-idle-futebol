      ...
      <div style={{display:"flex",flexDirection:"column",gap:7,marginBottom:20}}>
        {state.market.map((p:any)=>(
          <PlayerCard key={p.id} player={p} onAction={()=>doBuy(p)}
            actionLabel={priceLabel(fmt(p.price))} actionColor={state.coins>=p.price?colors.success:"#374151"}/>
        ))}
      </div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:5}}>
        {state.roster.map((p:any)=>(
          <PlayerCard key={p.id} player={p} compact onAction={()=>doSell(p)}
            actionLabel={priceLabel(fmt(p.sellPrice))} actionColor={colors.danger}/>
        ))}
