...
export function fieldLayout(formation:FormationKey, isHome:boolean, W:number, H:number): FieldSlot[] {
  ...
  const slots: FieldSlot[] = [{x:isHome?W*0.055:W*0.945, y:H*0.5, num:1, role:"GOL", line:0}];
  parts.forEach((cnt,si)=>{
    const role: PositionKey = si===0 ? "ZAG" : si===parts.length-1 ? "ATA" : "MEI";
    const xp = isHome ? 0.17+(si+1)*(0.62/(parts.length+1)) : 0.83-(si+1)*(0.62/(parts.length+1));
    for(let i=0;i<cnt;i++) slots.push({x:W*xp, y:H*((i+1)/(cnt+1)), num:slots.length+1, role, line:si+1});
  });
  return slots;
}
